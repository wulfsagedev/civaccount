...
const NORTH_STAR_22 = [
  ...
  'Broadland', 'Bromsgrove', 'Broxbourne', 'Cannock Chase',
];
...
// Evidence-image fingerprint manifest (tamper-evidence for screenshots).
// Generated by scripts/generate-image-manifest.mjs; lives in the private
// data repo so it versions together with the PNGs. Every cited PNG is
// re-hashed against it — a post-capture edit to an evidence image fails CI.
import { createHash } from 'node:crypto';
const IMAGE_MANIFEST_PATH = join(DC, 'image-manifest.json');
const IMAGE_MANIFEST = existsSync(IMAGE_MANIFEST_PATH)
  ? JSON.parse(readFileSync(IMAGE_MANIFEST_PATH, 'utf8'))
  : null;

function verifyImageFingerprint(pngPath, relKey) {
  if (!IMAGE_MANIFEST) return { ok: null, reason: 'no image-manifest.json (run npm run generate:image-manifest)' };
  const expected = IMAGE_MANIFEST[relKey];
  if (!expected) return { ok: false, reason: `not in image-manifest.json — run npm run generate:image-manifest and commit it` };
  const actual = createHash('sha256').update(readFileSync(pngPath)).digest('hex');
  if (actual !== expected.sha256) {
    return { ok: false, reason: `screenshot fingerprint mismatch — file changed since manifest (expected ${expected.sha256.slice(0, 12)}…, got ${actual.slice(0, 12)}…)` };
  }
  return { ok: true };
}
...
for (const name of NORTH_STAR_22) {
  ...
  for (const [field, entry] of withPng) {
    // 1. PNG on disk?
    const relKey = entry.page_image_url.replace(/^\/archive\//, '');
    const pngPath = join(DC, 'pdfs', 'council-pdfs', relKey);
    const pngExists = existsSync(pngPath);
    // 1b. PNG untampered? Re-hash against image-manifest.json.
    const fingerprint = pngExists ? verifyImageFingerprint(pngPath, relKey) : { ok: null };
    ...
    perEntry.push({ field, pngExists, fingerprint, scalarValue, excerpt: entry.excerpt, verify });
    if (!pngExists) councilOk = false;
    if (fingerprint.ok === false) councilOk = false;
  ...
  if (!councilOk) {
    ...
    for (const p of perEntry) {
      ...
      if (p.fingerprint?.ok === false) console.log(`     · ${p.field}: ${p.fingerprint.reason}`);
