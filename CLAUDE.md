...
- Re-run `enrich-waste-kpis.py` (uses substring matching that gave 36 wrong results)
- Give districts `waste_destinations` (they handle collection, not disposal)

---

## Data Enrichment — Kent Parity Project

### Goal
Bring all 317 English council data entries to "Kent parity" — matching Kent County Council's gold standard of ~47 field groups per council entry.

### Kent Reference
Kent County Council is the gold-standard council entry at line ~3052 of `county-councils.ts`. It has ~542 lines covering all 47 field groups. Every council should match this level of detail.

### Council Data Files

| File | Councils | Lines | Avg Parity |
|------|----------|-------|------------|
| `src/data/councils/county-councils.ts` | 21 | ~7,300 | 88.5% |
| `src/data/councils/districts.ts` | 164 | ~37,000 | 73.0% |
| `src/data/councils/metropolitan.ts` | 36 | ~11,300 | 77.7% |
...
**Council Tax**: `council_tax` (5 years of Band D rates)
**Budget**: `budget` (11 spending categories with descriptions)
**URLs**: `website`, `council_tax_url`, `accounts_url`, `transparency_url`, `councillors_url`, `budget_url`
**Leadership**: `council_leader`, `chief_executive`, `total_councillors`, `cabinet`
**Allowances**: `councillor_basic_allowance`, `total_allowances_cost`, `councillor_allowances_detail`
**Salary**: `chief_executive_salary`, `salary_bands`
**Workforce**: `staff_fte`
**Transparency**: `governance_transparency`, `section_transparency`, `documents`, `open_data_links`, `sources`
**Services**: `service_outcomes`, `service_spending`
**Suppliers**: `top_suppliers` (20 suppliers with descriptions)
**Environment**: `waste_destinations`
**Performance**: `performance_kpis`
**Financial**: `budget_gap`, `savings_target`, `grant_payments`
...
|-------|----------|-------|
| `performance_kpis` | 317/317 (100%) | ✅ Done |
| `section_transparency` | 317/317 (100%) | ✅ Done |
| `documents` | 317/317 (100%) | ✅ Done |
| `open_data_links` | 317/317 (100%) | ✅ Done |
| `cabinet` | 317/317 (100%) | ✅ Done (0 placeholders) |
| `chief_executive_salary` | 317/317 (100%) | ✅ Done |
| `councillor_basic_allowance` | 317/317 (100%) | ✅ Done |
| `website` + all URL fields | 317/317 (100%) | ✅ Done |
| `governance_transparency` | 317/317 (100%) | ✅ Done |
| `waste_destinations` | 153/153 (100%) | ✅ Done (non-district councils only) |
| `salary_bands` | 305/317 (96%) | 12 remaining — blocked/unavailable |
| `total_allowances_cost` | 303/317 (96%) | 14 remaining — blocked/unavailable |
| `savings_target` | 313/317 (99%) | ✅ Near-complete |
| `staff_fte` | 311/317 (98%) | ✅ Near-complete |
...
- **Budget values**: stored in THOUSANDS (£k) — e.g., `amount: 1500` means £1.5m
- **Council tax**: stored in POUNDS — e.g., `band_d: 1842.66`
- **Supplier annual_spend**: stored in POUNDS — e.g., `annual_spend: 45000000`
- **ServiceDetail type**: `{ name: string; description: string; }` — description is REQUIRED
- **CabinetMember type**: `{ name: string; role: string; portfolio: string; party?: string; }`
- **performance_kpis.status**: `"green" | "amber" | "red"`
- **Document.type**: `"other" | "accounts" | "budget" | "strategy" | "audit" | "report"`

### Council Entry Regex Pattern

The correct regex to find council entries in TS files:
```python
r'\n  \{\n    ons_code: "[^"]+",\n    name: "([^"]+)"'
```

### District Council Exemptions

Districts handle waste COLLECTION, not DISPOSAL. They correctly have:
- NO `waste_destinations` (DEFRA data only covers disposal authorities)
- NO education/social care service_outcomes

### Joint Waste Authorities

Some councils share waste disposal via joint authorities (E50xxx ONS codes):
- E50000001: East London WA (Barking+Dagenham, Havering, Newham, Redbridge)
- E50000002: North London WA (Barnet, Camden, Enfield, Hackney, Haringey, Islington, Waltham Forest)
- E50000003: West London WA (Brent, Ealing, Harrow, Hillingdon, Hounslow, Richmond upon Thames)
- E50000004: Western Riverside WA (Hammersmith+Fulham, Kensington+Chelsea, Lambeth, Wandsworth)
- E50000005: Greater Manchester WDA (Bolton, Bury, Oldham, Rochdale, Salford, Stockport, Tameside, Trafford)
- E50000006: Merseyside WDA (Knowsley, Liverpool, Sefton, St Helens, Wirral)

### Enrichment Scripts (all in project root)

| Script | Fields | Status |
|--------|--------|--------|
| `enrich-workforce.py` | `staff_fte` | ✅ Run (+300 councils) |
| `enrich-waste-kpis.py` | `waste_destinations`, `performance_kpis` | ✅ Run (DO NOT re-run — substring matching) |
| `enrich-top-suppliers.py` | `top_suppliers` | ✅ Run (301/317) |
| `enrich-kpis-derived.py` | `performance_kpis` for districts | ✅ Run (+166 councils) |
| `enrich-waste-remaining.py` | `waste_destinations` gaps | ✅ Run (+41 councils) |
| `enrich-cabinet.py` | `cabinet` from ModernGov | ✅ Run (0 new) |
| `scrape-cabinet-websites.py` | `cabinet` from websites | ✅ Run (+4 councils) |
| `enrich-urls.py` | URLs | ✅ Run (0 new) |
| `enrich-financial-strategy.py` | `budget_gap`, `savings_target` | ✅ Run |
| `enrich-transparency-links.py` | transparency links | ✅ Run |
| `enrich-documents-sources.py` | `documents`, `open_data_links` | ✅ Run |
| `audit-kent-parity.py` | Gap analysis (CSV + console) | ✅ Created |
| `cleanup-bad-cabinet.py` | Removed 16 garbage cabinet entries | ✅ Run |

### Supporting Data Files

- `council-websites.csv` — 317 council website URLs
- `src/data/councils/pdfs/gov-uk-bulk-data/moderngov-councils.csv` — 52 ModernGov URLs
- `src/data/councils/pdfs/gov-uk-bulk-data/discovered-urls.csv` — 182 councils with transparency/councillors/budget URLs
- `src/data/councils/pdfs/gov-uk-bulk-data/defra-waste-2022-23.ods` — DEFRA waste data
- `kent-parity-audit.csv` — Latest gap analysis output

### Remaining Manual Work (Phases 3-7)

All remaining enrichment requires MANUAL per-council research on `.gov.uk` websites:

**Phase 3**: Allowances & Salary Bands
- `councillor_basic_allowance` — Members' Allowances Scheme
- `total_allowances_cost` — Total allowances paid
- `salary_bands` — Senior salary disclosure (£5k bands from £50k+)
- `chief_executive_salary` — Pay policy statement

**Phase 4**: Per-Councillor Allowance Breakdowns
- `councillor_allowances_detail` — Individual councillor allowances

**Phase 5**: Supplier Descriptions & Named Contracts
- ~6,340 supplier descriptions to hand-write

**Phase 6**: Deep Open Data Links
- Expand from 3 to 9 themes per council

**Phase 7**: Financial Strategy & Final Gaps

**Processing order**: County Councils → Unitary Authorities → Metropolitan Districts → London Boroughs → District Councils (alphabetical, batches of 5)

### Where to Find Data on Council Websites

- Allowances: `{council}.gov.uk/councillors/allowances` or `/transparency`
- Salary bands: `{council}.gov.uk/about/pay-policy` or `/transparency`
- CEO salary: Pay policy statement (usually PDF)
- Cabinet: `{council}.gov.uk/councillors/cabinet` or ModernGov portal
- Budget: `{council}.gov.uk/about/budget` or `/finances`

### Important Technical Notes

1. **V2.0 is NOT a git repository** — no version control. Be careful with edits.
2. **Always run `npx tsc --noEmit` after data changes** to verify TypeScript compilation.
3. **41 existing cabinet entries have placeholder portfolios** ("Cabinet Member" not actual portfolio) — need fixing.
4. **ModernGov `.gov.uk` portals DON'T serve allowance data** — all return 404 for `mgAllowancesReport.aspx`.
5. **The plan file is at**: `/Users/owenfisher/.claude/plans/keen-bubbling-manatee.md`
