...
| **Adur** | SD | 🟢 **North-Star complete (v1.3 strict)** | All phases 0-7 ✓ **+ Phase 5b ✓ + derivation-strip ✓** | 2026-05-01 | Fourth fully-compliant reference council and the **first district**. First council to flag a personnel-currency drift in Phase 3.6 (CE: Catherine Howe → Paul Brewer; Leader: stripped → Cllr Jeremy Gardner) — Adur changed political control 23 May 2024 (Labour gain) and CE during FY 2025/26. 6 Tier-3 PDFs archived + 11 page-image PNGs + full Datasheet-for-Datasets audit (`docs/ADUR-AUDIT.md`) + `manifests/adur.json` + `status/adur.json`. **0/5 north-star gaps. 0 UX-audit violations (0 unwrapped + 0 derived).** New strips: `chief_executive_salary` (£166,626 was Howe's 2024-25 salary, cannot misattribute to Brewer); UI strip of two-tier district-explainer callout in `YourBillCard.tsx` (`£2,068 = totalBill - thisCouncilBandD` derivation — same precedent as YoY removal). Fixed: population 64,200 → 64,889 (verbatim ONS Mid-2024). 6-member Cabinet populated with verbatim portfolios from democracy.adur-worthing.gov.uk Cmt 138 (via Wayback — Cloudflare-blocked). |
...
- **Councils North-Star complete: 139**
  ...
  - **Batches 32-43 (35 SDs)** — flagged for **remediation**: shipped before the v1.3 depth-bar was enforced. Need full 14-phase Bradford-level rollout. Districts: Adur (✅ remediated 2026-05-01 — see reference table above), Bassetlaw, Braintree, Brentwood, Burnley, Cambridge, Chelmsford, Cherwell, Chesterfield, Chorley, Colchester, Crawley, Eastleigh, Fenland, Gedling, Stevenage, Staffordshire Moorlands, Guildford, Havant, High Peak, Mid Sussex, Norwich, North Norfolk, North Devon, Three Rivers, Winchester, Welwyn Hatfield, Sevenoaks, Spelthorne, Tewkesbury, Stratford-on-Avon, North West Leicestershire, Forest of Dean, Hyndburn, Ipswich.
- Councils in progress: 0
- Councils not yet started: 178

**v1.3 remediation queue (2026-05-01 reset):** Per the user reset directive, batches 32-43 (35 districts) shipped breadth-first with only partial depth (CE name+salary plus blanket strips, missing inventory, AUDIT.md, manifest, live UX sweep). They MUST be re-rolled-out one at a time, full 14 phases each. Adur is the first complete v1.3 district. The remaining 34 will be processed in alphabetical order (Bassetlaw next), one council per session, with no breadth-shipping.
