...
import EVIDENCE_IMAGES from './evidence-images.json';
...
/**
 * Visual evidence for a Tier-1 (GOV.UK spreadsheet) field: a rendered
 * image of the council's actual row from the archived, fingerprinted
 * reference file — generated by scripts/council-research/
 * render-csv-evidence.mjs. The compact index lists which (council,
 * field) pairs have an image; the filename is derived from the field
 * path (same rule as fileFor() in the renderer — keep in sync).
 */
function tier1EvidenceImage(fieldPath: string, council?: Council): string | undefined {
  if (!council?.name) return undefined;
  const idx = EVIDENCE_IMAGES as { _fields: string[] } & Record<string, number[] | string[]>;
  const vi = idx._fields?.indexOf(fieldPath);
  if (vi === undefined || vi === -1) return undefined;
  const slug = generateSlug(council.name);
  const list = idx[slug];
  if (!Array.isArray(list) || !(list as number[]).includes(vi)) return undefined;
  return `/archive/${slug}/images/${fieldPath.replace(/\./g, '_')}-row.png`;
}
...
export function getProvenance(
  ...
  if (FIELD_PROVENANCE[fieldPath]) {
    ...
    // Visual evidence: the council's rendered row from the archived
    // GOV.UK reference file (spreadsheets have no "page" to photograph).
    prov.page_image_url = prov.page_image_url ?? tier1EvidenceImage(fieldPath, council);
    return prov;
  }
