...
export function renderGrants(council: Council, councilName: string): ReactElement {
            ...
            return (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <span style={{ fontSize: '42px', fontWeight: 600, color: OG.text }}>{g.recipient}</span>
                  <span style={{ fontSize: '42px', fontWeight: 700, color: OG.text }}>{formatCurrencyOG(g.amount, 0)}</span>
                </div>
