...
export default function InsightsClient() {
  ...
  const stats = useMemo(() => {
    ...
    // CEO salary stats
    const ceoSalaries = councils
      .filter(c => c.detailed?.chief_executive_salary && c.detailed.chief_executive_salary > 0)
      .map(c => ({ council: c, salary: c.detailed!.chief_executive_salary! }))
      .sort((a, b) => b.salary - a.salary);
    const avgCeoSalary = ceoSalaries.length > 0
      ? ceoSalaries.reduce((sum, c) => sum + c.salary, 0) / ceoSalaries.length
      : 0;
    const highestCeoSalaries = ceoSalaries.slice(0, 5);
    const lowestCeoSalaries = [...ceoSalaries].sort((a, b) => a.salary - b.salary).slice(0, 5);

    // Councillor allowance stats
    const allowances = councils
      .filter(c => c.detailed?.councillor_basic_allowance && c.detailed.councillor_basic_allowance > 0)
      .map(c => ({ council: c, allowance: c.detailed!.councillor_basic_allowance! }))
      .sort((a, b) => b.allowance - a.allowance);
    const avgAllowance = allowances.length > 0
      ? allowances.reduce((sum, c) => sum + c.allowance, 0) / allowances.length
      : 0;

    // Service outcomes benchmarks
    const benchmarks = getServiceOutcomeBenchmarks();

    // Financial health
    const councilsWithReserves = councils
      .filter(c => c.detailed?.reserves && c.detailed.reserves > 0)
      .map(c => ({ council: c, reserves: c.detailed!.reserves! }));
    const councilsWithBudgetGap = councils
      .filter(c => c.detailed?.budget_gap && c.detailed.budget_gap > 0)
      .map(c => ({ council: c, gap: c.detailed!.budget_gap! }))
      .sort((a, b) => b.gap - a.gap);
    const totalReserves = councilsWithReserves.reduce((sum, c) => sum + c.reserves, 0);
    const totalBudgetGaps = councilsWithBudgetGap.reduce((sum, c) => sum + c.gap, 0);
    const biggestBudgetGaps = councilsWithBudgetGap.slice(0, 5);
    ...
    return {
      ...
      totalPopulation,
      // CEO & allowances
      ceoSalaries: ceoSalaries.length,
      avgCeoSalary,
      highestCeoSalaries,
      lowestCeoSalaries,
      avgAllowance,
      ...
      // Financial health
      totalReserves,
      councilsWithReserves: councilsWithReserves.length,
      biggestBudgetGaps,
      councilsWithBudgetGap: councilsWithBudgetGap.length,
      totalBudgetGaps,
    };
  }, []);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main id="main-content" className="flex-1">
        <div className="container mx-auto px-4 py-8 sm:px-6 sm:py-12 max-w-5xl">
            ...
            return (
              <div className="p-4 rounded-xl bg-muted/30 border border-border/50 mb-8">
                <p className="type-body-sm">
                  <span className="font-semibold">{name}</span> charges{' '}
          ...
          <div className="card-elevated p-6 sm:p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="type-body-sm text-muted-foreground">Rankings grouped by council type for fair comparison</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="type-body-sm font-medium">Biggest increases</span>
                </div>
                <div className="space-y-2">
                  {stats.biggestIncreases.map((item, i) => (
                    <div key={item.council.ons_code} className="flex items-center justify-between p-3 rounded-xl bg-muted/30">
                      <div className="flex items-center gap-3">
                        <span className="type-body-sm text-muted-foreground w-4 tabular-nums">{i + 1}.</span>
                        <Link href={`/council/${getCouncilSlug(item.council)}`} className="type-body-sm font-medium truncate hover:underline cursor-pointer !min-h-0">{item.council.name}</Link>
                        <Badge variant="outline" className="type-body-sm hidden sm:inline-flex">{item.council.type_name}</Badge>
                      </div>
                      <Badge variant="outline" className="type-body-sm tabular-nums shrink-0">+{item.percentChange.toFixed(1)}%</Badge>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="type-body-sm font-medium">Smallest increases</span>
                </div>
                <div className="space-y-2">
                  {stats.smallestIncreases.map((item, i) => (
                    <div key={item.council.ons_code} className="flex items-center justify-between p-3 rounded-xl bg-muted/30">
                      ...
                      <Badge variant="outline" className="type-body-sm bg-navy-50 text-navy-600 border-navy-200 dark:bg-navy-950/30 dark:text-navy-400 dark:border-navy-800 tabular-nums shrink-0">
          ...
          <div className="card-elevated p-6 sm:p-8 mb-8">
            ...
            <div className="space-y-4">
              {stats.priceBands.map((band) => {
                ...
                return (
                  <div key={band.label}>
                    <div className="flex items-center justify-between type-body-sm mb-2">
                      <span className="font-medium">{band.label}</span>
                      <span className="text-muted-foreground tabular-nums">{band.count} {band.count === 1 ? 'council' : 'councils'} ({percentage.toFixed(0)}%)</span>
                    </div>
                ...
                <div key={type} className="p-4 border border-border/50 rounded-xl">
                  ...
                  <div className="space-y-2 type-body-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold tabular-nums">{formatCurrency(data.avgBandD, { decimals: 0 })}</span>
                    </div>
                    <div className="flex justify-between">
          ...
          <div className="card-elevated p-6 sm:p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="type-body-sm text-muted-foreground">Combined spending across all {stats.councilsWithBudget} councils with budget data</p>
              </div>
            </div>
                ...
                return (
                  <div key={service.key}>
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <span className="type-body-sm font-medium">{SERVICE_NAMES[service.key]}</span>
                        <p className="type-body-sm text-muted-foreground">{info?.description}</p>
                      </div>
                      <div className="text-right shrink-0 ml-4">
                        <span className="font-semibold">{formatBillions(service.value)}</span>
                        <span className="text-muted-foreground ml-2 type-body-sm tabular-nums">({percentage.toFixed(0)}%)</span>
                      </div>
                    </div>
                    ...
                    {info?.includes && (
                      <div className="flex flex-wrap gap-1.5">
                        {info.includes.slice(0, 4).map((item) => (
                          <Badge key={item} variant="outline" className="type-body-sm font-normal">
                            {item}
                          </Badge>
                        ))}
                        {info.includes.length > 4 && (
                          <Badge variant="outline" className="type-body-sm font-normal text-muted-foreground">
            ...
            <div className="card-elevated p-6 sm:p-8 mb-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="type-body-sm text-muted-foreground">How much councils spend per person and on administration</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <div className="p-4 bg-muted/30 rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="type-body-sm font-medium">Average per person</span>
                  </div>
                  <p className="type-metric font-semibold tabular-nums">
                ...
                <div className="p-4 bg-muted/30 rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="type-body-sm font-medium">Admin overhead</span>
                  </div>
                  <p className="type-metric font-semibold tabular-nums">
                ...
                <div className="p-4 bg-muted/30 rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="type-body-sm font-medium">Population covered</span>
                  </div>
                  <p className="type-metric font-semibold tabular-nums">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingDown className="h-4 w-4 text-muted-foreground" />
                    <span className="type-body-sm font-medium">Lowest spending per person</span>
                  </div>
                  <div className="space-y-2">
                    {stats.efficiencyStats.lowestPerCapita.map((item, i) => (
                      <div key={item.council.ons_code} className="flex items-center justify-between p-3 rounded-xl bg-muted/30">
                        <div className="flex items-center gap-3 min-w-0">
                          <span className="type-body-sm text-muted-foreground w-4 tabular-nums shrink-0">{i + 1}.</span>
                          <Link href={`/council/${getCouncilSlug(item.council)}`} className="type-body-sm font-medium hover:underline cursor-pointer !min-h-0">{item.council.name}</Link>
                          <Badge variant="outline" className="type-body-sm">{item.council.type_name}</Badge>
                        </div>
                        <span className="font-semibold tabular-nums">{formatCurrency(item.perCapitaSpending, { decimals: 0 })}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingUp className="h-4 w-4 text-muted-foreground" />
                    <span className="type-body-sm font-medium">Highest spending per person</span>
                  </div>
                  <div className="space-y-2">
              ...
              <div className="mt-6 pt-6 border-t border-border/50">
                <h4 className="type-body-sm font-semibold mb-4 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <CheckCircle className="h-4 w-4 text-muted-foreground" />
                      <span className="type-body-sm font-medium">Lowest admin overhead</span>
                    </div>
                    <div className="space-y-2">
                      {stats.efficiencyStats.lowestAdminOverhead.map((item, i) => (
                        <div key={item.council.ons_code} className="flex items-center justify-between p-3 rounded-xl bg-muted/30">
                          ...
                          <Badge variant="outline" className="type-body-sm bg-navy-50 text-navy-600 border-navy-200 dark:bg-navy-950/30 dark:text-navy-400 dark:border-navy-800 tabular-nums">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <AlertTriangle className="h-4 w-4 text-muted-foreground" />
                      <span className="type-body-sm font-medium">Highest admin overhead</span>
                    </div>
                    <div className="space-y-2">
                      {stats.efficiencyStats.highestAdminOverhead.map((item, i) => (
                        <div key={item.council.ons_code} className="flex items-center justify-between p-3 rounded-xl bg-muted/30">
                          ...
                          <Badge variant="outline" className="type-body-sm tabular-nums">{item.adminOverheadPercent.toFixed(1)}%</Badge>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="card-elevated p-6 sm:p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="type-body-sm text-muted-foreground">Different councils have very different spending priorities</p>
              </div>
            </div>
                    ...
                    <div key={type} className="p-4 border border-border/50 rounded-xl">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          ...
                          <p className="type-body-sm text-muted-foreground">{data.count} councils</p>
                        </div>
                        <Badge variant="outline" className="type-body-sm">{formatBillions(data.total)} total</Badge>
                      </div>
                      ...
                      <div className="space-y-3 mb-3">
                        {topServices.map(([service, amount]) => {
                          ...
                          return (
                            <div key={service}>
                              <div className="flex items-center justify-between type-body-sm mb-1">
          ...
          {/* Service Outcomes — what councils actually achieve */}
          <div className="card-elevated p-6 sm:p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center">
              ...
              <div>
                <h2 className="type-title-2">What councils achieve</h2>
                <p className="type-body-sm text-muted-foreground">Service outcomes across England</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="p-4 rounded-xl bg-muted/30">
                <p className="type-caption text-muted-foreground mb-1">Average recycling rate</p>
                <p className="type-metric font-semibold tabular-nums">{stats.benchmarks.recycling.nationalAverage.toFixed(1)}%</p>
                <p className="type-caption text-muted-foreground mt-1">{stats.benchmarks.recycling.count} councils reporting</p>
              </div>

              {/* Homes built */}
              <div className="p-4 rounded-xl bg-muted/30">
                <p className="type-caption text-muted-foreground mb-1">Homes built last year</p>
                <p className="type-metric font-semibold tabular-nums">{stats.benchmarks.housing.nationalTotal.toLocaleString('en-GB')}</p>
                <p className="type-caption text-muted-foreground mt-1">{stats.benchmarks.housing.count} councils reporting</p>
              </div>

              {/* Ofsted */}
              <div className="p-4 rounded-xl bg-muted/30">
                <p className="type-caption text-muted-foreground mb-1">Children&apos;s services rated Good or Outstanding</p>
                <p className="type-metric font-semibold tabular-nums">
                  {stats.benchmarks.ofsted.total > 0
                    ? Math.round(((stats.benchmarks.ofsted.distribution['Outstanding'] || 0) + (stats.benchmarks.ofsted.distribution['Good'] || 0)) / stats.benchmarks.ofsted.total * 100)
                    : 0}%
                </p>
                <p className="type-caption text-muted-foreground mt-1">{stats.benchmarks.ofsted.total} councils inspected</p>
              </div>

              {/* Road condition */}
              {stats.benchmarks.roadCondition.count > 0 && (
                <div className="p-4 rounded-xl bg-muted/30">
                  <p className="type-caption text-muted-foreground mb-1">Roads in good condition</p>
                  <p className="type-metric font-semibold tabular-nums">{stats.benchmarks.roadCondition.nationalAverage.toFixed(0)}%</p>
                  <p className="type-caption text-muted-foreground mt-1">{stats.benchmarks.roadCondition.count} highway authorities</p>
                </div>
              )}
            </div>
          </div>
          ...
          {stats.ceoSalaries > 0 && (
            <div className="card-elevated p-6 sm:p-8 mb-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="type-title-2">Executive pay</h2>
                  <p className="type-body-sm text-muted-foreground">Chief executive salaries across {stats.ceoSalaries} councils</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="p-4 rounded-xl bg-muted/30">
                  <p className="type-caption text-muted-foreground mb-1">Average CEO salary</p>
                  <p className="type-metric font-semibold tabular-nums">{formatCurrency(stats.avgCeoSalary, { decimals: 0 })}</p>
                </div>
                <div className="p-4 rounded-xl bg-muted/30">
                  <p className="type-caption text-muted-foreground mb-1">Average councillor allowance</p>
                  <p className="type-metric font-semibold tabular-nums">{formatCurrency(stats.avgAllowance, { decimals: 0 })}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingUp className="h-4 w-4 text-muted-foreground" />
                    <span className="type-body-sm font-medium">Highest paid</span>
                  </div>
                  <div className="space-y-2">
                    {stats.highestCeoSalaries.map((item, i) => (
                      <div key={item.council.ons_code} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30">
                        <span className="type-body-sm text-muted-foreground tabular-nums shrink-0">{i + 1}.</span>
                        <Link href={`/council/${getCouncilSlug(item.council)}`} className="type-body-sm font-medium truncate hover:underline cursor-pointer flex-1 min-w-0 !min-h-0">{item.council.name}</Link>
                        <span className="type-body-sm font-semibold tabular-nums shrink-0">{formatCurrency(item.salary, { decimals: 0 })}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingDown className="h-4 w-4 text-muted-foreground" />
                    <span className="type-body-sm font-medium">Lowest paid</span>
                  </div>
                  <div className="space-y-2">
                    {stats.lowestCeoSalaries.map((item, i) => (
                      <div key={item.council.ons_code} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30">
                        <span className="type-body-sm text-muted-foreground tabular-nums shrink-0">{i + 1}.</span>
                        <Link href={`/council/${getCouncilSlug(item.council)}`} className="type-body-sm font-medium truncate hover:underline cursor-pointer flex-1 min-w-0 !min-h-0">{item.council.name}</Link>
                        <span className="type-body-sm font-semibold tabular-nums shrink-0">{formatCurrency(item.salary, { decimals: 0 })}</span>
          ...
          {stats.councilsWithBudgetGap > 0 && (
            <div className="card-elevated p-6 sm:p-8 mb-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="type-title-2">Financial health</h2>
                  <p className="type-body-sm text-muted-foreground">Reserves, budget gaps, and financial pressure</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="p-4 rounded-xl bg-muted/30">
                  <p className="type-caption text-muted-foreground mb-1">Total reserves held</p>
                  <p className="type-metric font-semibold tabular-nums">£{(stats.totalReserves / 1_000_000_000).toFixed(1)}bn</p>
                  <p className="type-caption text-muted-foreground mt-1">{stats.councilsWithReserves} councils</p>
                </div>
                <div className="p-4 rounded-xl bg-muted/30">
                  <p className="type-caption text-muted-foreground mb-1">Combined budget gaps</p>
                  <p className="type-metric font-semibold tabular-nums">£{(stats.totalBudgetGaps / 1_000_000_000).toFixed(1)}bn</p>
                  <p className="type-caption text-muted-foreground mt-1">{stats.councilsWithBudgetGap} councils</p>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <AlertTriangle className="h-4 w-4 text-muted-foreground" />
                  <span className="type-body-sm font-medium">Largest budget gaps</span>
                </div>
                <div className="space-y-2">
                  {stats.biggestBudgetGaps.map((item, i) => (
                    <div key={item.council.ons_code} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30">
                      <span className="type-body-sm text-muted-foreground tabular-nums shrink-0">{i + 1}.</span>
                      <Link href={`/council/${getCouncilSlug(item.council)}`} className="type-body-sm font-medium truncate hover:underline cursor-pointer flex-1 min-w-0 !min-h-0">{item.council.name}</Link>
                      <span className="type-body-sm font-semibold tabular-nums shrink-0 text-negative">£{(item.gap / 1_000_000).toFixed(0)}m</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="card-elevated p-6 sm:p-8 mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 bg-muted/30 rounded-xl">
                <div className="flex items-center gap-2 mb-2">
                  <TrendingUp className="h-4 w-4 text-muted-foreground" />
                  <span className="type-body-sm font-semibold">Rising every year</span>
                </div>
                <p className="type-body-sm text-muted-foreground leading-relaxed">
              ...
              <div className="p-4 bg-muted/30 rounded-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="type-body-sm font-semibold">Location matters</span>
                </div>
                <p className="type-body-sm text-muted-foreground leading-relaxed">
              ...
              <div className="p-4 bg-muted/30 rounded-xl">
                <div className="flex items-center gap-2 mb-2">
                  <Users className="h-4 w-4 text-muted-foreground" />
                  <span className="type-body-sm font-semibold">Education & care dominate</span>
                </div>
                <p className="type-body-sm text-muted-foreground leading-relaxed">
