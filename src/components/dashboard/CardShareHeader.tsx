...
export default function CardShareHeader({
  ...
  const shareUrl = slug ? withReferral(`${SITE_URL}/council/${slug}/card/${cardType}`, 'share', cardType) : undefined;
  const imageUrl = slug ? `/api/share/${slug}/${cardType}?format=story` : undefined;
  const embedUrl = slug ? withReferral(`${SITE_URL}/embed/council/${slug}/${cardType}`, 'embed', cardType) : undefined;
